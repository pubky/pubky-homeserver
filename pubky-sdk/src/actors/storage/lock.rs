...
/// let state = storage.get(lock.path()).await?.bytes().await?;
/// // ... modify `state` ...
/// storage.put_locked(&lock, state).await?;
...
impl SessionStorage {
    ...
    /// `PUT` `body` to the path of a lock this client holds.
    ///
    /// # Errors
    /// A lock that has expired or was unlocked returns 412; take a new one and
