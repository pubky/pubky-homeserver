...
pub(super) fn blob_fingerprint(metadata: &Metadata) -> Option<String> {
    ...
    metadata.last_modified().map(|modified| {
        format!(
            "modified:{}:{}",
            modified.to_rfc3339_opts(SecondsFormat::Nanos, true),
...
mod tests {
    ...
    fn fingerprint_prefers_etag_then_version_then_modification_time() {
        ...
        metadata.set_last_modified(chrono::DateTime::from_timestamp_nanos(1_000_000_001));
        assert_eq!(
            blob_fingerprint(&metadata).as_deref(),
            Some("modified:1970-01-01T00:00:01.000000001Z:7")
        );
