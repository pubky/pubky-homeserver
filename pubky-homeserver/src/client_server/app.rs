...
pub fn create_app(state: AppState) -> std::result::Result<Router, ClientServerBuildError> {
    ...
    Ok(with_trace_layer(app)
        ...
        .layer(CorsLayer::very_permissive().expose_headers([RETRY_AFTER, ETAG])))
}
...
mod tests {
    ...
    #[tokio::test]
    #[pubky_test_utils::test]
    async fn cors_exposes_retry_after_and_etag_headers() {
        ...
        response.assert_header(header::ACCESS_CONTROL_EXPOSE_HEADERS, "retry-after,etag");
    }
    ...
    async fn info_is_public_and_reports_features() {
        ...
        response.assert_json(&serde_json::json!({
            "features": ["path-addressed-storage", "conditional-writes"]
        }));
    }
