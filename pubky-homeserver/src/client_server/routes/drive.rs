...
//! and delete.
//!
//! It is served from the homeserver on purpose. `/dav` does answer cross-origin
//! requests, so a page hosted elsewhere could call it — but served from here the
//! explorer is same-origin, which means no preflight round trip and, more
//! usefully, that it works on a homeserver with no public origin at all.
//!
