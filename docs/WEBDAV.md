...
*This describes what the endpoint does. For what is still outstanding see
[WEBDAV-ROADMAP.md](./WEBDAV-ROADMAP.md); for why it is built this way, the
pre-implementation design record is at [WEBDAV-DESIGN.md](./WEBDAV-DESIGN.md).*
...
- [Clients That Work](#clients-that-work)
...
The homeserver can serve a file explorer at **`/drive`**. It is **off by default** —
set `drive.web_explorer = true` to enable it. Sign in with a public key and token and
you get a Drive-style view of the same files: folders, image thumbnails, text
preview, upload, delete and download.
...
Storage quotas, bandwidth quotas and request rate limits all apply here exactly as
they do to the REST routes — the endpoint sits behind the same middleware and writes
through the same storage layer. The shipped config also rate-limits `PROPFIND` per
user, generously enough for a real sync client.

Authorization matches the REST routes exactly. Every request is checked against the
token's capabilities for the path it targets, so a token scoped to `/pub/someapp/`
can read and write there and nowhere else, and `MOVE`/`COPY` destinations are
checked separately from their sources. The one concession to how clients work is
that the drive root and the `/pub/` and `/priv/` directories stay listable by their
owner whatever the token's scope — a client has to list them to mount anything, and
they reveal only the shape every drive shares.
...
[drive]
webdav = true        # default
web_explorer = false # default; requires webdav

[admin]
demo_users = false   # default
```

All three default safely and may be omitted, so a `config.toml` written before they
...
Requires `admin.demo_users = true`; it is off by default. On a homeserver run for
people to try things out, the admin API can provision a
...
└── pubky.app/          ← social data
```

`pub/pubky.app/` holds a synthetic social profile in the
[pubky-app-specs](https://github.com/pubky/pubky-app-specs) shapes — a profile,
six posts (including a reply and a repost), tags, a bookmark, follows, a mute, a
feed and an image chain of `posts` → `files` → `blobs`. The content is invented, so
no real account's posts ship in the binary.

Both kinds of identifier are generated per user rather than baked in: timestamp IDs
must fall inside the window the spec accepts, and hash IDs are Blake3 digests of
URIs that embed the drive owner's public key. Two demo users therefore never share
a tag or bookmark path.
...
Any WebDAV client should work. These are the ones that have actually been checked.

**Verified against a deployed homeserver:** `curl`, `rclone`, GNOME Files and
Thunar (both through GVfs), Filestash, and the built-in `/drive` explorer.

**macOS Finder:** everything Finder needs is verified on the wire — `DAV: 1,2,3`,
the `LOCK` → `PUT` → `UNLOCK` write sequence, and its housekeeping files — including
through a TLS reverse proxy. The GUI itself has not been driven.

**Expected to work:** KDE Dolphin (`webdavs://`), Cyberduck, Mountain Duck, WinSCP,
`davfs2`, Windows Explorer.

**Apps that can use a drive as their storage backend**, with no Pubky integration:
Joplin, Obsidian (via the Remotely Save plugin), Zotero, KeePassXC, Duplicati,
Documents by Readdle (iOS), Solid Explorer (Android). Point one at your drive and
its files live on your homeserver — and anything under `/pub/` is readable on the
open web.

**Will not work:** `rsync` (not a WebDAV client, though it is fine against a drive
you have already mounted), Time Machine (SMB/AFP only), and the Nextcloud/ownCloud
desktop clients, which speak Nextcloud's API rather than plain WebDAV.

Prefer Cyberduck on Windows: Explorer's built-in WebDAV client has a default ~50 MB
file cap and long-standing auth quirks.
...
- **A drive is reachable only by its owner.** `/pub/` is world-readable over the
  REST API, but WebDAV requires authentication and refuses another user's drive
  outright, so you cannot yet mount someone else's public folder.
...
- **Uploads are capped at 100MB**, the same as the REST routes; a larger body gets
  a `413`.
- **Empty folders are invisible outside WebDAV.** `MKCOL` creates a collection that
  `PROPFIND` will list, but directories are implied by key prefixes rather than
  stored, so nothing appears in the event feed or the REST listing until a file
  lands in it.
- **File managers leave junk, and it is refused.** Finder writes `.DS_Store` and
  `._` files, GNOME writes `.Trash-$UID`. Anything outside `/pub/` and `/priv/`
  gets a `403`, the same as over REST, which macOS may occasionally surface as an
  error dialog. That is deliberate: the alternative is accepting files the rest of
  the system considers invalid.
